...
<head>
    ...
    <title>Tính diện tích hình học bằng PHP</title>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <link rel="stylesheet" href="styles.css">
    <script>
        function onShapeChange(selected) {
            // Ẩn tất cả khối input
            document.querySelectorAll('[id^="inputs-"]').forEach(function(el){
                el.classList.add('hidden');
            });
            // Hiện khối theo tên hình
            var target = document.getElementById('inputs-' + selected);
            if (target) target.classList.remove('hidden');
        }

        // Khởi tạo theo lựa chọn đã gửi ở lần trước (nếu có)
        document.addEventListener('DOMContentLoaded', function () {
            const checked = document.querySelector('input[name="shape"]:checked');
            if (checked) onShapeChange(checked.value);
        });

        // Chặn Enter gây submit ngoài ý muốn khi nhập liệu
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Enter' && e.target.tagName === 'INPUT') {
                const isButton = e.target.type === 'submit' || e.target.type === 'button';
